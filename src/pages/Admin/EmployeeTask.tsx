...
const EmployeeDetail = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://127.0.0.1:8000/api/employee/${id}/tasks/`, {
        headers: {
          Authorization: `Token ${token}`,
        },
      })
  ...
  const handleSaveTaskRating = (taskId: number) => {
    ...
    if (task) {
      axios
        .post(
          `http://127.0.0.1:8000/api/task/${taskId}/rate/`,
          {
            rating: task.rating,
          },
          {
  ...
  const handleSaveAttributeRatings = () => {
    axios
      .post(
        `http://127.0.0.1:8000/api/employee/${id}/attributes/`,
        {
    ...
    <div className="container mt-5">
          ...
          <Col key={task.id} md={4} className="mb-3">
            <div className="card">
              <div className="card-body">
                ...
                <p>
                  <strong>Appraisable:</strong>{" "}
                  {task.is_appraisable ? "Yes" : "No"}
                </p>
                <br />
                <Form.Control
                  type="number"
                  ...
                  value={task.rating}
                  onChange={(e) =>
                    handleTaskRatingChange(task.id, parseInt(e.target.value))
                  }
                />
                <Button
                  className="mr-2 mt-3"
                  onClick={() => handleSaveTaskRating(task.id)}
                >
                  Save Ratings
                </Button>
              </div>
      ...
      <Modal show={showAttributesModal} onHide={handleCloseAttributesModal}>
            ...
            {[...Array(10)].map((_, index) => (
              <Col key={index} md={4} className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    handleAttributeRatingChange(index, parseInt(e.target.value))
                  }
                />
              </Col>
            ))}
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSaveAttributeRatings}>
            Save Ratings
          </Button>
        </Modal.Footer>
      </Modal>
